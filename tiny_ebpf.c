...
int ebpf_interpreter(struct ebpf_context *ctx) {
    ...
    if (MODE == EBPF_MODE_INT_DEBUG)
        printf("Run interrupt debug mode\n");
    // R1O end of the stack
    ctx->R[10] = sizeof(ctx->stack);
    // Program counter
    uint32_t PC = 0;
    uint32_t cycles = 0;
    for (;cycles < 2048; PC++, cycles++) {
        ...
        if (class == BPF_LD || class == BPF_LDX ||
            ...
            uint8_t sz_bits = 64;
            if (sz == BPF_W) sz_bits = 32;
            if (sz == BPF_H) sz_bits= 16;
            if (sz == BPF_B) sz_bits = 8;
            // uint64_t *dst_reg = &ctx->R[dst_reg];
            // uint64_t *src_reg = &ctx->R[src_reg];
            if (class == BPF_LDX && lopcode == BPF_MEM) {
                uint32_t addr = ctx->R[src_reg] + offset;
                if (MODE)
                    printf("MEM r%d = *(u%d *) r%d + %d // %u\n", dst_reg, sz_bits, src_reg, offset, addr);
                if (MODE == EBPF_MODE_DISASM)
                    continue;
                uint8_t *dst_byte = (uint8_t *) &ctx->R[dst_reg];
                uint16_t i = 0;
                for (; i < sz_bits/8; i++) {
                    // printf("%d: r%d (%d) = stack[%d] (%d)\n", i, dst_reg, dst_byte[i], addr+i, ctx->stack[addr+i]);
                    dst_byte[i] = ctx->stack[addr + i];
                }
                for (; i < sizeof(ctx->R[dst_reg]); i++)
                    dst_byte[i] = 0;
                ...
                uint32_t addr = ctx->R[dst_reg] + offset;
                if (MODE)
                    printf("MEM *(u%d *) r%d + %d = %d // %u\n", sz_bits, dst_reg, offset, imm, addr);
                if (MODE == EBPF_MODE_DISASM)
                    continue;
                // Write to stack
                uint8_t *src_byte = (uint8_t *) &imm;
                for (uint16_t i = 0; i < sz_bits/8; i++) {
                    // printf("%d = %d\n", addr+i, src_byte[i]);
                    ctx->stack[addr + i] = src_byte[i];
                }
            } else if (class == BPF_STX && lopcode == BPF_MEM) {
                uint32_t addr = ctx->R[dst_reg] + offset;
                if (MODE)
                    printf("MEM *(u%d *) r%d + %d = r%d // %u <- %lu\n", sz_bits, dst_reg, offset, src_reg, addr, ctx->R[src_reg]);
                if (MODE == EBPF_MODE_DISASM)
                    continue;
                // Write to stack
                uint8_t *src_byte = (uint8_t *) &ctx->R[src_reg];
                for (uint16_t i = 0; i < sz_bits/8; i++) {
                    // printf("%d: stack[%d] (%d) = r%d (%d)\n", i, addr+i, ctx->stack[addr + i], src_reg, src_byte[i]);
                    ctx->stack[addr + i] = src_byte[i];
                }
            } else {
                if (MODE)
